#!/usr/bin/env python3
"""
BetPredict Pro — Daily Data Fetcher (v6.1)
==========================================
Fix v6.1:
  - Predicții: fetch GLOBAL (fără filtru ligă) → 200+ predicții
  - Paginare automată pentru toate endpoint-urile
  - Fallback per-ligă dacă global fetch returnează 0
  - Edge calculation fix pentru cote estimate
  - Git: commit înainte de rebase (în workflow)
"""
...
LEAGUES = {
    ...
    13: "Scottish Prem",       17: "Saudi Pro League",  9: "Brasileirao",
    25: "Ekstraklasa",         14: "Pro League BE",    15: "Super League CH",
    19: "Liga MX",             22: "Parva Liga",       24: "Super League GR",
    26: "Allsvenskan",         11: "Veikkausliiga",    28: "Allsvenskan",
    16: "Superlig Turcia",     20: "Jupiler League",   21: "Liga 1 Romania",
}

# ── Strategii backtestale din VEYRA ─────────────────────────────────────────
STRATEGIES = {
    "engine_overall": {
        "label": "Engine Overall", "icon": "🎯",
        "markets": ["homeWin","draw","awayWin","over15","under25","under35"],
        "min_adj": 66.0, "min_conf": 0.0, "min_edge": 8.0,
        "odd_min": 1.15, "odd_max": 1.65, "color": "#00e87a",
    },
    "best_single": {
        "label": "Evenimentul zilei", "icon": "⭐",
        "markets": ["homeWin","over15","over25","under35"],
        "min_adj": 72.0, "min_conf": 50.0, "min_edge": 8.0,
        "odd_min": 1.20, "odd_max": 1.95, "color": "#ffb830",
    },
    "conservative": {
        "label": "Bilet conservator", "icon": "🛡️",
        "markets": ["over15","under35"],
        "min_adj": 74.0, "min_conf": 50.0, "min_edge": 5.0,
        "odd_min": 1.12, "odd_max": 1.65, "color": "#4a9eff",
    },
    "smart_ev": {
        "label": "Smart EV", "icon": "💡",
        "markets": ["homeWin","awayWin","over15","over25","under35"],
        "min_adj": 66.0, "min_conf": 0.0, "min_edge": 2.0,
        "odd_min": 1.20, "odd_max": 2.20, "color": "#a78bfa",
    },
    "over15_specialist": {
        "label": "Over 1.5G Specialist", "icon": "⚽",
        "markets": ["over15"],
        "min_adj": 76.0, "min_conf": 50.0, "min_edge": 8.0,
        "odd_min": 1.43, "odd_max": 1.60, "color": "#22d3ee",
    },
}

MARKET_LABELS = {
    "homeWin": "1 Victorie acasă",  "draw":    "X Egal",
    "awayWin": "2 Victorie deplasare", "over15": "Over 1.5G",
    "over25":  "Over 2.5G",         "under25": "Under 2.5G",
...
def get_all_pages(url, params=None, max_pages=20):
    """
    Fetch cu paginare automată.
    Returnează lista completă de rezultate din toate paginile.
    """
    params = params or {}
    ...
    while page_url and page_url not in seen_urls and page < max_pages:
        ...
        if batch:
            print(f"    pag {page}: +{len(batch)} (total {len(results)})")
...
def save(filename, data, protect_empty=True):
    ...
    keys_to_check = ("results", "signals", "events", "standings")
    if isinstance(data, dict):
        new_cnt = len(next((data[k] for k in keys_to_check if k in data), []))
    else:
        new_cnt = len(data)
    ...
    if protect_empty and new_cnt == 0 and path.exists():
        try:
            ...
            old_cnt = len(next((old[k] for k in keys_to_check if k in old), []) if isinstance(old, dict) else old)
            if old_cnt > 0:
                print(f"  SKIP {filename} (0 nou, pastrez {old_cnt})")
                return
        except: pass
...
def get_league_name(league_id):
    return LEAGUES.get(int(league_id) if league_id else 0, f"Liga {league_id}")
...
def normalize_pred(p, fallback_lid=None, fallback_lname=""):
    ...
    # Liga — BSD v2 returnează league_id și league_name direct pe event (FLAT)
    lid   = ev.get("league_id") or fallback_lid
    lname = ev.get("league_name") or get_league_name(lid) or fallback_lname
    p["_league_id"]   = lid
    ...
    # Over/Under la multiple threshold-uri (0-100 → 0-1)
    ...
    for pred in preds:
        ...
        for strat_key, strat in STRATEGIES.items():
            for market_key in strat["markets"]:
                ...
                if strat["min_conf"] > 0 and conf > 0 and conf < strat["min_conf"]: continue

                # Cotă: reală (bookmaker) sau estimată no-vig
                def get_odd(mk, prob):
                    if mk in ("homeWin", "draw", "awayWin") and bo:
                        fm = {"homeWin": ["home_odds","odds_1"], "draw": ["draw_odds","odds_x"], "awayWin": ["away_odds","odds_2"]}
                        for f in fm.get(mk, []):
                            v = bo.get(f)
                            if v and float(v) > 1: return float(v), True
                    return round(1 / (prob * 1.05), 2) if prob > 0.05 else 0, False

                odd, is_real = get_odd(market_key, prob01)
                if odd < strat["odd_min"] or odd > strat["odd_max"]: continue

                ev_val = prob01 * odd - 1

                # Edge calculation:
                # Cu cote reale: edge vs no-vig bookmaker
                # Fără cote: pseudo-edge față de baseline fair (33.3% pt 1X2, 50% pt binary)
                if is_real:
                    edge_pp = (prob01 - 1/odd) * 100 if odd > 1 else 0
                else:
                    is_binary = market_key not in ("homeWin", "draw", "awayWin")
                    fair_baseline = 0.50 if is_binary else 0.333
                    edge_pp = (prob01 - fair_baseline) * 100
...
# ─────────────────────────────────────────────────────────────────────────────
# [1/6] PREDICTIONS — fetch global cu paginare → 200+ predicții
# ─────────────────────────────────────────────────────────────────────────────
def fetch_predictions():
    print("\n[1/6] Predictions (global fetch cu paginare)...")
    all_preds, seen = [], set()

    # ── Strategie 1: GLOBAL FETCH (fără filtru de ligă) ──
    # Returnează 200+ predicții pentru toate ligile disponibile
    fetched_globally = False
    for base in [BASE_V2, BASE_V1]:
        url = f"{base}/predictions/"
        print(f"  → încearcă {url} (global, fără filtru)...")
        raw = get(url)  # fără params → returnează toate predicțiile
        ...
        page = 1
        print(f"    pag {page}: {len(batch)} predicții")

        while next_url and page < 30:
            next_data = get(next_url)
            if not next_data: break
            new_batch = next_data.get("results", [])
            batch.extend(new_batch)
            page += 1
            print(f"    pag {page}: +{len(new_batch)} (total {len(batch)})")
            next_url = next_data.get("next")

        if not batch:
            continue

        # Normalizare și enrichment
        for p in batch:
            pid = p.get("id")
        ...
        if all_preds:
            fetched_globally = True
            print(f"  global OK: {len(all_preds)} predicții totale")
            break

    # ── Strategie 2: FALLBACK per ligă (dacă global returnează 0) ──
    if not fetched_globally:
        print("  fallback: fetch per ligă...")
    ...
    print(f"  TOTAL: {len(all_preds)} pred | prob:{n_p} | grade:{n_g} | poisson:{n_poi}")
...
def fetch_matches_today():
    ...
    # Încearcă fetch global pentru meciurile de azi
    all_m = get_all_pages(f"{BASE_V2}/events/", {
    ...
    if not all_m:
        # Fallback per ligă
        for lid, lname in LEAGUES.items():
            ms = get_all_pages(f"{BASE_V2}/events/", {
                "date_from": today, "date_to": today, "league_id": lid, "limit": 50
            })
            if not ms:
                ms = get_all_pages(f"{BASE_V2}/events/", {"league_id": lid, "status": "notstarted", "limit": 20})
            for m in ms:
...
def fetch_standings():
    print("\n[4/6] Clasamente...")
    data = {}
    for lid, lname in list(LEAGUES.items())[:12]:  # primele 12 ligi principale
        for url, params in [
...
def compute_signals():
    ...
    if not pred_path.exists():
        save("signals.json", {"updated_at": now_iso(), "count": 0, "signals": [], "by_strategy": {}, "strategy_stats": {}}, protect_empty=False)
        return
    ...
    counts_str = " | ".join(f"{sk}:{len(v)}" for sk, v in by_strat.items())
    print(f"  TOTAL: {len(signals)} semnale | {counts_str}")

    save("signals.json", {
        "updated_at": now_iso(), "count": len(signals),
        "signals": signals, "by_strategy": by_strat, "strategy_stats": strat_stats
    }, protect_empty=True)
...
def compute_value_bets():
    ...
    for pred in preds:
        ...
        bo = (odds_idx.get(eid) or {}).get("1x2", {})
        has_real = bool(bo)
        raw = [bo.get("home_odds") or bo.get("odds_1"),
...
if __name__ == "__main__":
    if not API_KEY:
        ...
        sys.exit(1)
    print(f"=== BetPredict Pro v6.1 — {today_iso()} (analytics: {'YES' if HAS_AC else 'NO'}) ===")
    fetch_predictions()
    fetch_matches_today()
    fetch_best_odds()
    fetch_standings()
