#!/usr/bin/env python3
"""BetPredict Event Deep Data v1.

Implementează endpointurile BSD API v2 pentru subresursele unui eveniment:
- /api/v2/events/{id}/stats/
- /api/v2/events/{id}/lineups/
- /api/v2/events/{id}/player-stats/
- /api/v2/events/{id}/shotmap/
- /api/v2/events/{id}/incidents/

Design:
- rulează după fetch_daily.py, când predictions/signals/value_bets/match_context există;
- selectează evenimente prioritare, nu toată baza globală;
- nu oprește workflow-ul dacă un endpoint este 404/empty;
- scrie atomic fișierele JSON;
...
def compact_count(payload: Any) -> int:
    ...
    if isinstance(payload, dict):
        for key in ("results", "events", "items", "data", "stats", "lineups", "players", "incidents", "shots"):
...
def short_payload(payload: Any, max_rows: int = 8) -> Any:
    if isinstance(payload, dict):
        out = dict(payload)
        for key in ("results", "events", "items", "data", "stats", "lineups", "players", "incidents", "shots"):
...
def request_json(path: str, params: Optional[Dict[str, Any]] = None, label: str = "") -> Tuple[Any, Dict[str, Any]]:
    ...
    headers = {
        ...
        "User-Agent": "BetPredict-Event-Deep-Data/1.0",
    }
    if API_KEY:
...
def add_event(events: Dict[str, Dict[str, Any]], event_id: Any, row: Dict[str, Any], source: str, score: float) -> None:
    ...
    item = events.setdefault(key, {
        ...
        "away_team": away,
        "league": league,
        "league_id": ev.get("league_id") or row.get("league_id"),
        "event_date": ev.get("event_date") or ev.get("start_time") or row.get("event_date") or row.get("start_time"),
        "sources": [],
        "priority_score": 0.0,
    })
...
def normalize_event_resource(event: Dict[str, Any], resource: str, payload: Any, meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "results": rows,
        "raw": payload if not rows and isinstance(payload, dict) else None,
    }


def main() -> int:
    print("\n[Event Deep Data v1] Event subresources BSD v2...")
    events = seed_priority_events(EVENT_LIMIT)
    reports: List[Dict[str, Any]] = []
    combined: List[Dict[str, Any]] = []
    buckets: Dict[str, List[Dict[str, Any]]] = {key: [] for key in SUBRESOURCES}

    if not events:
        write_json(DEBUG_DIR / "event_deep_data_debug.json", {
            "updated_at": now_iso(),
            "source": "event_deep_data_v1",
            "error": "no_priority_events",
            "limit": EVENT_LIMIT,
        })
    ...
    for idx, event in enumerate(events, 1):
        ...
        for resource, endpoint_name in SUBRESOURCES.items():
            ...
            normalized = normalize_event_resource(event, resource, payload, meta)
            buckets[resource].append(normalized)
            event_row["resources"][resource] = {
                "ok": meta.get("ok"),
                "status": meta.get("status"),
                "count": meta.get("count"),
                "sample": short_payload(payload, max_rows=3),
            }
    ...
    for resource, rows in buckets.items():
        per_resource[resource] = {
            "events": len(rows),
            "ok_with_data": sum(1 for r in rows if r.get("ok") and int(r.get("count") or 0) > 0),
            "ok_empty": sum(1 for r in rows if r.get("ok") and int(r.get("count") or 0) == 0),
            "not_found": sum(1 for r in rows if r.get("status") == 404),
            "errors": sum(1 for r in rows if not r.get("ok") and r.get("status") not in (404, 401, 403)),
            "total_rows": sum(int(r.get("count") or 0) for r in rows if r.get("ok")),
        }

    summary = {
        "events_requested": len(events),
        "resources_per_event": len(SUBRESOURCES),
        "requests_total": len(reports),
        "ok_with_data": sum(1 for r in reports if r.get("ok") and int(r.get("count") or 0) > 0),
        "ok_empty": sum(1 for r in reports if r.get("ok") and int(r.get("count") or 0) == 0),
        "not_found": sum(1 for r in reports if r.get("status") == 404),
        "auth_failed": sum(1 for r in reports if r.get("status") in (401, 403)),
        "errors": sum(1 for r in reports if not r.get("ok") and r.get("status") not in (404, 401, 403)),
        "per_resource": per_resource,
    }
    updated_at = now_iso()

    write_json(DATA_DIR / "event_deep_data.json", {
    ...
    file_map = {
        ...
        "lineups": "event_lineups.json",
        "player_stats": "event_player_stats.json",
        "shotmap": "event_shotmap.json",
        "incidents": "event_incidents.json",
    }
    for resource, filename in file_map.items():
        rows = buckets[resource]
        write_json(DATA_DIR / filename, {
            "updated_at": updated_at,
            "source": f"event_{resource}_v1",
            "count": len(rows),
            "summary": per_resource.get(resource, {}),
            "results": rows,
        })
    ...
    write_json(DEBUG_DIR / "event_deep_data_debug.json", {
        ...
        "limits": {
            ...
            "http_sleep": HTTP_SLEEP,
        },
        "summary": summary,
        "reports": reports[:300],
        "sample_events": combined[:10],
    })

    print(
        "  event deep data: "
        f"events={len(events)} requests={len(reports)} "
        f"ok_data={summary['ok_with_data']} empty={summary['ok_empty']} "
        f"404={summary['not_found']} errors={summary['errors']}"
    )
    return 0
